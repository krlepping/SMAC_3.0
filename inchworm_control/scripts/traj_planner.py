...
class TrajectoryPlanner:
    ...
    def eval_quintic_pose(coeffs, t):
        return coeffs[0] + coeffs[1] * t + coeffs[2] * t ** 2 + coeffs[3] * t ** 3 + coeffs[4] * t ** 4 + coeffs[
            5] * t ** 5
    ...
    def get_joint_state(self):
        ...
        joint_names = [f"iw_ankle_foot_bottom_{self.idx}", f"iw_beam_ankle_bottom_{self.idx}",
                       f"iw_mid_joint_{self.idx}", f"iw_beam_ankle_top_{self.idx}", f"iw_ankle_foot_top_{self.idx}"]
        cur_angles = []
    ...
    '''
        ...
        if self.real_robot:
            joint_names = [f"iw_ankle_foot_bottom", f"iw_beam_ankle_bottom", f"iw_mid_joint", f"iw_beam_ankle_top",
                           f"iw_ankle_foot_top"]
        else:
            joint_names = [f"iw_ankle_foot_bottom_{self.idx}", f"iw_beam_ankle_bottom_{self.idx}",
                           f"iw_mid_joint_{self.idx}", f"iw_beam_ankle_top_{self.idx}", f"iw_ankle_foot_top_{self.idx}"]
